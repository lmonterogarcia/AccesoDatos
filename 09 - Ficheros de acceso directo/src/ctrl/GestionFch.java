...
public class GestionFch {
	...
	public void escribirRegistro(int iNumRegistro, Persona persona) {
		...
		try {
			...
			char cCaracter;
			for (int i = 0; i < model.Persona.getbNumCaracteresNombrePersona(); i++) {
	...
	private long getPosition(int iNumRegistro) {
		...
		return (iNumRegistro - 1) * model.IPersona.iLongRegistro;
	}

	public Persona leerRegistro(int iNumRegistro) {
		String sNombre = "";
		byte bEdad = 0;
		float fAltura = 0.0f;
		...
		try {
			...
			for (int i = 0; i < model.IPersona.bNumCaracteresNombrePersona; i++) {
		...
		return new Persona(sNombre.trim(),bEdad,fAltura);
	}
}
